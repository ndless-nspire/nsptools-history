...
{
    unsigned char color = g->color >> 4;
    unsigned char colormask = color | (color << 4);

    volatile char *fb = g->drawfb + g->fbwidth * y + (x >> 1);
    ...
    if (((x + len - 1) ^ x) < 2)
    {
         ...
         */
        unsigned char mask = ((1 << (len << 2)) - 1) << ((x & 1) << 2);
    ...
    else
    {
        ...
        {
            int beginlen = 2 - (x & 1);
            unsigned char mask = ((1 << (beginlen << 2)) - 1) << ((x & 1) << 2);
            if (g->mode == HPG_MODE_PAINT) *fb &= ~mask;
            *(fb++) ^= mask & colormask;
        }
        ...
        int endlen = ((len + (x & 1) - 1) & 1) + 1;
        unsigned char mask = (1 << (endlen << 2)) - 1;
        if (g->mode == HPG_MODE_PAINT) *fb &= ~mask;
        *fb ^= mask & colormask;
    }
}
...
static void draw_pattern(hpg_t *g, int x, int y, int len, int patx, int paty)
{
     ...
     */
    unsigned int masterpattern = 0x00;
    unsigned char srcmask = 0x01;
    unsigned int dstmask = 0x0F;
    ...
    idx = ((x & ~1) - patx) & 7;
    masterpattern = (masterpattern >> (idx << 2))
        | (masterpattern << (32 - (idx << 2)));
    ...
    else
    {
        ...
        if (x & 1)
        {
            ...
            *(fb++) ^= mask & colormask;
            ...
            masterpattern = (masterpattern >> 8) | (masterpattern << 24);
        }
