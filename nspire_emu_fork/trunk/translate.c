...
static u8 insn_buffer[10000000];
static u8 *insn_bufptr = insn_buffer;
...
static inline void emit_push(int reg) {
	emit_byte(0x50 | reg);
}

static inline void emit_pop(int reg) {
	emit_byte(0x58 | reg);
}

static inline void emit_ret() {
	emit_byte(0xC3);
}
...
static inline void emit_long_jump(u32 target) {
	emit_byte(0xE9);
	emit_dword(target - ((u32)out + 4));
}
...
int translate(u32 start_pc) {
	...
	emit_push(EBX);
	emit_mov_x86reg_immediate(EBX, (int)&arm);

	emit_byte(0xFF);
	emit_byte(0xA1);
	emit_dword((u32)jtbl_bufptr - start_pc);
	...
	while (1) {
		...
		insnp = ram_ptr(pc, 4);
		if (!insnp) {
			printf("stopping translation - not in RAM %x\n", pc);
...
no_condition:
		...
		if ((insn & 0xE000090) == 0x0000090) {
			if ((insn & 0xFC000F0) == 0x0000090) {
				...
				if ((post_index && pre_index) || base_reg == 15 || data_reg == 15)
					goto unimpl;
				...
				if (insn & (1 << 22)) {
					...
					if (post_index) {
						...
						emit_alu_armreg_immediate(offset_op, base_reg, offset);
					} else if (pre_index) {
						emit_alu_armreg_immediate(offset_op, base_reg, offset);
						emit_mov_x86reg_armreg(ECX, base_reg);
					} else {
		...
		data_proc_done:
			...
			if (base_reg == 15 && (pre_index || post_index))
				break;
			...
			if (insn & (1 << 25)) {
				...
				if (base_reg == 15)
					...
					emit_mov_x86reg_armreg(ECX, base_reg);
				if (count == 0 && !post_index) {
					emit_alu_x86reg_armreg(offset_op, ECX, offset_reg);
				} else {
					emit_mov_x86reg_armreg(EAX, offset_reg);
					emit_shift_x86reg(shift_table[shift_type], EAX, count);
					if (post_index)
						emit_alu_armreg_x86reg(offset_op, base_reg, EAX);
					else
						emit_alu_x86reg_x86reg(offset_op, ECX, EAX);
				}
			} else {
				// Offset is immediate
			...
			if (pre_index)
				emit_mov_armreg_x86reg(base_reg, ECX);

			if (is_load) {
				/* LDR/LDRB instruction */
				u32 proc = is_byteop ? (u32)read_byte : (u32)read_word;
				if (data_reg == 15) {
					emit_pop(EBX);
					emit_long_jump(proc);
					stop_here = 1;
			...
			/* Load/store multiple */
			int before    = insn & (1 << 24);
			int delta     = insn & (1 << 23) ? +1 : -1;
			int special   = insn & (1 << 22);
			int writeback = insn & (1 << 21);
			int load      = insn & (1 << 20);

			if (special) // restore CPSR, or use umode regs
				goto unimpl;

			u32 addr_reg = insn >> 16 & 15;
			if (addr_reg == 15)
				goto unimpl;

			int reg = (delta >= 0) ? 0 : 15;

			int offset = 0;
			emit_push(ESI);
			emit_mov_x86reg_armreg(ESI, addr_reg);
			for (reg = (delta >= 0) ? 0 : 15; !(reg & ~15); reg += delta) {
				if (!(insn >> reg & 1))
					continue;
				if (before) offset += delta << 2;
				emit_byte(0x8D); // LEA
				emit_modrm_base_offset(ECX, ESI, offset);
				if (load) {
					if (reg == 15) {
						if (delta < 0) {
							// PC is first read when decreasing; save it
			...
			if (writeback)
				emit_alu_armreg_immediate(ADD, addr_reg, offset);

			if (insn & (1 << 15) && load) {
				// LDM with PC
				if (delta < 0) {
					// It was the first thing read; we pushed it on the stack
					emit_pop(EAX);
				}
				emit_pop(ESI);
				emit_pop(EBX);
				if (delta >= 0) // Increasing LDM loads PC last.
					emit_long_jump((u32)read_word);
				else
					emit_ret();
				stop_here = 1;
			} else {
...
branch_unconditional:
	...
	translation_table[index].code       = (procptr)insn_bufptr;
	translation_table[index].jump_table = (u32)jtbl_bufptr - start_pc;
	translation_table[index].start_addr = start_pc;
	translation_table[index].end_addr   = pc;

	insn_bufptr = out;
	jtbl_bufptr = outj;

	if (insn_bufptr >= &insn_buffer[sizeof insn_buffer / sizeof *insn_buffer])
		error("Out of instruction space");
	if (jtbl_bufptr >= &jtbl_buffer[sizeof jtbl_buffer / sizeof *jtbl_buffer])
...
void flush_translations() {
	...
	for (index = 0; index < next_index; index++) {
		u32 start = translation_table[index].start_addr;
		u32 end   = translation_table[index].end_addr;
		u32 *flags = &RAM_FLAGS(ram_ptr(start, 4));
		for (; start < end; start += 4)
			*flags++ &= ~(RF_CODE_TRANSLATED | (-1 << RFS_TRANSLATION_INDEX));
	}
	next_index = 0;
