...
static u32 get_shifted_immed(int insn, int setcc) {
	int count = insn >> 7 & 30;
	s32 val = insn & 0xFF;
...
static int get_shifted_reg(u32 *ret, int insn, int setcc) {
	...
	*ret = shift(type, res, count, setcc);
	return 0;
}
...
void cpu_interpret_instruction(u32 insn) {
	...
	if ((insn & 0xE000090) == 0x0000090) {
		...
		else if (get_shifted_reg(&right, insn, setcc) == -1)
			return;
		...
		switch (insn & 0xEF00EF) {
			case 0x010000: { /* MCR p15, 0, <Rd>, c1, c0, 0: Control Register */
				...
				if ((value & 0xFFFF8FF8) != 0x00050078)
...
static inline void *get_pc_ptr(u32 pc, u32 align) {
	void *ptr = &addr_cache_delta[pc >> 10][pc];
	if ((u32)ptr & (AC_INVALID_PTR | (align - 1))) {
		ptr = addr_cache_miss(pc, align - 1, prefetch_abort);
		if (!ptr)
			error("Bad PC: %08x\n", pc);
	}
	return ptr;
...
void *cpu_save_state(size_t *size) {
	*size = sizeof(struct cpu_saved_state);
	struct cpu_saved_state *state = malloc(*size);
	memcpy(&state->arm, &arm, sizeof(arm));
	return state;
}

void cpu_reload_state(void *state) {
	struct cpu_saved_state *_state = (struct cpu_saved_state *)state;
	memcpy(&arm, &_state->arm, sizeof(arm));
}
