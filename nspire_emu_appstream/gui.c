...
// returns 1 if virtual keyboard must be updated
int gui_handle_key_press(int col, int row, int down) {
	...
	else if ((key_map[row] >> col & 1) != down) {
		key_map[row] ^= 1 << col;
		return 1;
	}
	return 0;
...
LRESULT CALLBACK keys_wnd_proc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_KEYDOWN:
	case WM_KEYUP: {
		if (gui_vkey_to_row_col(wParam, lParam, &col, &row, &down))
			break;
update:
		if (gui_handle_key_press(col, row, down)) {
			rc.left = col * KEY_WIDTH; rc.right = rc.left + KEY_WIDTH;
			rc.top = row * KEY_HEIGHT; rc.bottom = rc.top + KEY_HEIGHT;
